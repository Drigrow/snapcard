...
export const SearchPromptBar: React.FC<SearchPromptBarProps> = ({
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4">
      {/* Container Box */}
      <div className="glass-panel rounded-2xl sm:rounded-3xl p-3 sm:p-5 shadow-2xl transition-all border border-slate-200/80 dark:border-slate-800/80 hover:border-brand-500/40 dark:hover:border-brand-500/40 relative">
        ...
        <div className="relative">
          <textarea
            ...
            rows={2}
            className="w-full bg-transparent resize-none outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-sm sm:text-base font-normal leading-relaxed pr-24 py-1"
          />
          ...
          <div className="absolute right-1 bottom-2 flex items-center gap-1.5">
            ...
            <button
              ...
              title={t.photoUpload}
              className="p-2 rounded-xl text-slate-500 dark:text-slate-400 hover:text-brand-500 dark:hover:text-brand-400 hover:bg-slate-100 dark:hover:bg-slate-800/80 transition-colors"
            >
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-500 hover:to-indigo-500 text-white font-medium text-xs sm:text-sm flex items-center gap-1.5 shadow-md shadow-brand-500/20 disabled:opacity-40 disabled:cursor-not-allowed transition-all transform active:scale-95"
            >
              {isLoading ? (
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800/60 flex flex-wrap items-center justify-between gap-2.5 text-xs">
          
          {/* Audience selector */}
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-slate-500 font-medium hidden sm:inline">{t.audienceLabel}:</span>
          ...
          {/* Image Mode Switch */}
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-slate-500 font-medium hidden sm:inline">{t.imageModeLabel}:</span>
            <div className="flex items-center p-0.5 bg-slate-100 dark:bg-slate-900 rounded-lg border border-slate-200/60 dark:border-slate-800">
              <button
                type="button"
        ...
        <div className="flex items-center gap-1.5 flex-nowrap">
          {inspirationQueries.map((item, idx) => (
            <button
              ...
              disabled={isLoading}
              className="px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-900/80 hover:bg-brand-500/10 text-slate-600 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 border border-slate-200/60 dark:border-slate-800 hover:border-brand-500/30 whitespace-nowrap transition-all"
            >
              {item}
            </button>
          ))}
